  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    <div class="meta">
      <div class="meta-block">
        ...
        Client: ${q.clientName}${q.clientEmail ? ' · ' + q.clientEmail : ''}<br/>
        Date: ${new Date(q.createdAt?.toDate?.() || q.createdAt).toLocaleDateString('en-NA')}<br/>
    ...
    <div class="totals">
      ...
      ${q.taxPercent > 0 ? `<div class="total-row"><span>VAT (${q.taxPercent}%)</span><span>${sym} ${(q.taxAmount || 0).toFixed(2)}</span></div>` : ''}
      <div class="grand-total"><span>TOTAL DUE</span><span>${sym} ${(q.total || 0).toFixed(2)}</span></div>
    </div>
